...
from data_store_3 import DataStore
...
class BotInterface:
    def __init__(self, comunity_token, acces_token):
        ...
        self.db = DataStore(db_url_object)
        self.params = {}
        self.worksheets = []
        self.offset = 0
    ...
    def get_profile_user(self, worksheet):
                ...
                if command == 'привет':
                    ...
                    while True:
                        if self.worksheets:
                            ...
                            # Вызов функции, которая проверяет на наличие в базе данных анкеты
                            data_flag = self.get_profile_user(worksheet)
                            if data_flag is False:
                                self.db.insert_result(event.user_id, worksheet['id'])
                                break
                        else:
